document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateBackground() {
        ...
        document.body.style.setProperty('--background-start', `hsl(${currentPrimaryHue}, ${saturation + 10}%, ${lightness + 5}%)`);
        document.body.style.setProperty('--background-end', `hsl(${currentSecondaryHue}, ${saturation}%, ${lightness}%)`);
    ...
    const createHackerEffect = () => {
        const char = Math.random() < 0.5 ? String.fromCharCode(48 + Math.floor(Math.random() * 2)) : String.fromCharCode(0x2500 + Math.random() * 99); // Binary or Box Drawing chars
    ...
    function simulateAIResponse(userPrompt) {
        let aiResponse = "Hi, I am NOVA, Affan's personal AI coding assistant. I am still under development. ";
        let emojis = ["✨", "💡", "🤖", "🚀", "💻", "🧠", "🌟", "🛠️", "✅", "🎉"];
        let randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];

        if (userPrompt.toLowerCase().includes('hello') || userPrompt.toLowerCase().includes('hi')) {
            aiResponse = `Hello there! ${randomEmoji} How can I assist you with your coding journey today My HACKER?`;
...
This would give a light blue background, center-align the <h1> heading, and use the Arial font for the text. ${randomEmoji}`;
            ...
            aiResponse = `HTML, or HyperText Markup Language, is the standard markup language for documents designed to be displayed in a web browser. It can be assisted by technologies such as Cascading Style Sheets (CSS) and scripting languages such as JavaScript. ${randomEmoji}`;
             } else if (userPrompt.toLowerCase().includes('teach') || userPrompt.toLowerCase().includes('myself')) {
...
IF YOU WANT TO LEARN MORE ABOUT ANY PARTICULAR TOPIC PLEASE GO TO CHATGPT OR GEMINI. HACKER AFFAN.
${randomEmoji}`;
        } else if (userPrompt.toLowerCase().includes('made') || userPrompt.toLowerCase().includes('myself')) {
            aiResponse = `You are a Hacker. Your name is Affan Adil. And you are my creator. I am here to help you Every Time. ${randomEmoji}`;
        } else if (userPrompt.toLowerCase().includes('JavaScript') || userPrompt.toLowerCase().includes('js')) {
...
To sum up, little habits have a big impact. Society grows stronger and healthier when both adults and children adopt positive habits on a regular basis. Little actions now can have a significant effect later.
 ${randomEmoji}`;
            ...
            aiResponse = `Sorry for that Hacker, I'm still learning, but I'm ready to assist with basic of your coding queries. If your question is very important you can go to ChatGPT or Gemini like models for it. What specific problem are you trying to solve? ${randomEmoji}`;
        }
    ...
    function adjustInputHeight() {
        ...
        if (messageInput.scrollHeight > 100) { // Max height before showing scrollbar
    ...
    function highlightCode(code) {
        ...
        // Keywords
        highlightedCode = highlightedCode.replace(/\b(function|var|let|const|if|else|for|while|return|new|import|export|def|print)\b/g, '<span class="keyword">$&</span>');
        // Strings
        ...
        highlightedCode = highlightedCode.replace(/(=|\+|-|\*|\/|%|<|>|&|\||\!)/g, '<span class="operator">$&</span>');
    ...
    function addEmojis(text) {
        ...
        const emojiMap = {
            'code': '💻', 'coding': '💻', 'program': '👨‍💻', 'project': '📁',
            'idea': '💡', 'think': '🧠', 'error': '🐞', 'bug': '🐛',
            'fix': '🛠️', 'success': '✅', 'done': '✔️', 'happy': '😊',
            'awesome': '🤩', 'great': '👍', 'star': '🌟', 'rocket': '🚀',
            'hello': '👋', 'hi': '👋', 'bye': '👋', 'welcome': '🤝',
            'question': '❓', 'answer': '💡', 'explain': '🤔', 'learn': '📚',
            'data': '📊', 'security': '🔒', 'web': '🌐', 'mobile': '📱',
            'database': '🗄️', 'server': '🖥️', 'cloud': '☁️', 'api': '🔗',
            'test': '🧪', 'debug': '🔍', 'solution': '✔️', 'warning': '⚠️',
            'info': 'ℹ️', 'danger': '⛔', 'love': '❤️', 'sparkles': '✨',
            'smile': '😄', 'party': '🥳', 'celebrate': '🎉', 'robot': '🤖',
            'ai': '🧠', 'neural': '🧠', 'algorithm': '⚙️', 'flow': '🌊',
            'logic': '🧠', 'design': '🎨', 'frontend': '🖥️', 'backend': '⚙️',
            'database': '🗄️', 'deploy': '🚀', 'git': '🌳', 'github': '🐙'
        };
    ...
    document.getElementById('tempChatBtn').addEventListener('click', () => {
        alert('Starting a temporary chat. This conversation will not be saved Hacker.');
    ...
    document.getElementById('fileAccessBtn').addEventListener('click', () => {
        alert('File access feature coming soon! This will allow me to help with your local code files.');
